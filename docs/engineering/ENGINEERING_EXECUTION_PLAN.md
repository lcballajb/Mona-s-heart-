...
The first E4 increment now provides an executable application-operation contract and fills the in-memory adapter's missing operation surface. Sensitive tables with existing RLS policies force policy enforcement, closing PostgreSQL's table-owner bypass. Application operations for profiles, health entries, documents, and imported records now establish a transaction-local user context and reject reuse from a different user context; unit and PostgreSQL adversarial tests cover the fail-closed boundary. Organization-scoped and conversation-scoped operation coverage, return-shape, transaction, concurrency, lifecycle, and failure-semantics contract coverage remain next, followed by the rest of E3 tenant isolation. AI and agents must wait for existing gates; infrastructure, identity, database, authorization, audit and testing can proceed without weakening governance.
