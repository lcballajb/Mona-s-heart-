...
| `tests/postgres.integration.test.mjs` | migrations, seed idempotency, real adapter isolation and persistence, forced RLS catalog enforcement                                                                 | PostgreSQL 16                        | integration; contract; privacy; security; regression         | With test DB / yes | Locally when `TEST_DATABASE_URL` is absent; CI provides it and rejects production-like URLs  |
